...
enum Interest {
  Sports, Tech, Games, Mentoring, Art, Travel, Music, Reading, Cooking, Blogging
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  final titleFocusNodes = [FocusNode(), FocusNode(), FocusNode(), FocusNode(), FocusNode()];
      ...
      body: SafeArea(
        ...
        child: Form(
          ...
          child: ListView(
            ...
            children: <Widget>[
              MyToggleButtonsFormField<Title>(
                decoration: InputDecoration(
                  labelText: 'Title',
                ),
                initialValue: _formResult.title,
                items: Title.values,
                itemBuilder: (BuildContext context, Title titleItem) => Text(describeEnum(titleItem)),
                selectedItemBuilder: (BuildContext context, Title titleItem) => Text(describeEnum(titleItem)),
                onSaved: (title) {
                  _formResult.title = title;
                },
                borderRadius: BorderRadius.all(Radius.circular(5.0)),
                focusNodes: titleFocusNodes,
                onChanged: (title) {
                  final titleIndex = Title.values.indexOf(title);
                  if (titleIndex >= 0) {
                    FocusScope.of(context).unfocus();
                    FocusScope.of(context).requestFocus(titleFocusNodes[titleIndex]);
                  }
                },
              ),
              SizedBox(height: 8.0),
              ...
              MyTextFormField(
                ...
                inputFormatters: [LengthLimitingTextInputFormatter(30)],
                initialValue: _formResult.primaryNumber,
                validator: (number) {
                  if (number.isEmpty) {
                ...
                onSaved: (number) {
                  _formResult.primaryNumber = number;
                },
                autofocus: true,
                focusNode: nameFocusNode,
                textInputAction: TextInputAction.next,
              ...
              MyTextFormField(
                ...
                inputFormatters: [LengthLimitingTextInputFormatter(30)],
                initialValue: _formResult.primaryEmail,
                validator: (email) {
                  if (email.isEmpty) {
                ...
                onSaved: (email) {
                  _formResult.primaryEmail = email;
                },
                autofocus: true,
                focusNode: nameFocusNode,
                textInputAction: TextInputAction.next,
